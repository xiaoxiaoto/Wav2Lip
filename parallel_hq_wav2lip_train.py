    ...
    while global_epoch < nepochs:
        ...
        for step, (x, indiv_mels, mel, gt) in prog_bar:
            ...
            if global_step == 1 or global_step % checkpoint_interval == 0:
                checkpoint_path = join(checkpoint_dir, "wav2lip_step{:09d}_loss{:.4f}.pth".format(global_step,
                                                                                                  running_perceptual_loss / (
                                                                                                          step + 1)))
                save_checkpoint(model, optimizer, global_step, checkpoint_path, global_epoch)
                checkpoint_path = join(checkpoint_dir, "disc_wav2lip_step{:09d}_loss{:.4f}.pth".format(global_step,
                                                                                                       running_perceptual_loss / (
                                                                                                               step + 1)))
                ...
                with torch.no_grad():
                    ...
                    if average_sync_loss < global_loss:
                        checkpoint_path = join(checkpoint_dir,
                                               "bast_wav2lip_step{:09d}_loss{:.4f}.pth".format(global_step,
                                                                                               average_sync_loss))
                        save_checkpoint(model, optimizer, global_step, checkpoint_path, global_epoch)
